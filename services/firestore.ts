...
import type { UserProfile, HealthData, ClinicalAnalysis, NutritionalAnalysis, VigsCategory, SmokingStatus, VigsScore, NutriScore } from '../types';
...
const mapProfileToDb = (profile: Partial<UserProfile>) => {
    ...
    if (profile.healthData?.height !== undefined) {
        dbData.talla_cm = Number(profile.healthData.height) || 170;
    }
    return dbData;
};
...
export const saveNutritionLog = async (uid: string, analysis: NutritionalAnalysis): Promise<void> => {
    ...
    const { error } = await supabase.from('nutrition_logs').insert({ 
        user_id: uid, foto_url: analysis.imagePreview, comida_descripcion: `${nsPrefix}${analysis.analysis.portions}`,
...
export const getNutritionLogs = async (uid: string): Promise<NutritionalAnalysis[]> => {
    ...
    return (data || []).map(d => {
        ...
        const match = rawDesc.match(/\[NS:([A-E])\]/);
        if (match) {
            nutriScore = match[1] as NutriScore;
